...
for goal_detect_mean=0:0.2:2.5%0.1:0.9
...
n_trials=size(St_List,1);    
 ...
 Model_Drift=zeros(n_trials*n_rep,2);
...
[nodes_path,short]=shortestpath(G,St_Node,En_Node); %finds nodes path/shortest length
 ...
 while(Goal==0)  
     ...
     B_Mod(tr_r)=numel(find(Border_Map(C_Node)==1))/numel(Border_Map(C_Node));
    ...
    Model_Drift(tr_r,1) = max(Model_Drift(tr_r,1),dist_model); 
    Model_Drift(tr_r,2) = Model_Drift(tr_r,2) + dist_model;
...
%          end    
 ...
 Model_Drift(tr_r,2) = Model_Drift(tr_r,2)/Length;
...
Trial_Drift = Trial_Drift(:,2);
Model_Drift = Model_Drift(:,2);
...
para_search = [para_search; session tri goal_detect_mean,t_prob,h,p,k,KL,h_dr,p_dr,k_dr,KL_dr,B1,B2];

mm=mm+1;
 DistriComp{1,mm} = L_Ratio;
 DistriComp{2,mm} = DT_SP;
 DistriComp{3,mm} = Model_Drift;
 DistriComp{4,mm} = Trial_Drift;
 DistriComp{5,mm} = B_Mod;
 DistriComp{6,mm} = B_Beh;   
 ...
 if str_ch_pl==1
save(['parameter_fit_type_' num2str(typ) '_LongP_7'],'para_search','DistriComp')
 elseif str_ch_pl==2
   save(['parameter_fit_type_' num2str(typ) '_OptiP_7'],'para_search','DistriComp')  
